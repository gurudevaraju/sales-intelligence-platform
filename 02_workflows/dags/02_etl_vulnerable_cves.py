"""Airflow ETL DAG: s3://<bucket>/cssip/vulnerable_cves.csv -> cyber.vulnerable_cves.
...
tell the same 6-step story every run:
    ...
    5. LOAD      -- overwrite cyber.vulnerable_cves (create-if-missing +
...
Note: this is the largest source file (~2 GB). The chunked schema-inference
below handles it without OOM, but the worker still needs:
  * ~2 GB free on /tmp for the downloaded CSV, AND
  * ~2-3 GB free RAM per chunk during the pandas read.
...
TABLE_NAME = "vulnerable_cves"
S3_KEY = "cssip/vulnerable_cves.csv"
...
@dag(
    dag_id="02_etl_vulnerable_cves",
    description=f"S3 -> RDS ETL: {S3_KEY} -> {TARGET_SCHEMA_DEFAULT}.{TABLE_NAME}",
    schedule=None,
    start_date=datetime(2026, 1, 1),
    catchup=False,
    default_args=DEFAULT_ARGS,
    tags=["etl", "aws", "s3", "rds", "cyber", TABLE_NAME, "large-file"],
)
def s3_to_rds_etl_vulnerable_cves():
    @task
    def extract() -> str:
